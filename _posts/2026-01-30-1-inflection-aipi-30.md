...
2.  **推論プロセスの最適化:** 生成されるトークン1つ一つをいかに効率よく計算し、出力していくか。これには、量子化（Quantization）と呼ばれる、モデルのパラメータをより低い精度（例：FP16からINT8）で表現することでメモリ使用量と計算量を削減する技術や、プルーニング（Pruning）による不要な接続の削除、そして高度なコンパイル技術が関わってきます。Llama.cppのようなオープンソースプロジェクトが示すように、推論最適化は今、業界全体が力を入れている分野です。
